(function (window) {
    ...
    MyPromise.all = function (myPromises) {
        ...
        return new MyPromise((resolve, reject) => {
            myPromises.forEach(p => {
                // 参数 myPromises 的元素可以不为 promise 对象
                // 当其不为promise对象时，需要使用resolve方法将其包装为一个promise对象
    ...
    MyPromise.race = function (myPromises) {
        return new MyPromise((resolve, reject) => {
            myPromises.forEach(p => {
                // 一旦出现成功，当前promise就返回成功
                // 一旦出现失败，当前promise就返回失败
                // 由于状态只能改变一次，因此即使之后还调用了回调函数 ，也只会执行和返回第一个返回的结果
